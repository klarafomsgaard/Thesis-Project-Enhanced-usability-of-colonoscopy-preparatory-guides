---
title: "App data dem and analysis"
...
## DATA PREP
This section contains data cleaning and preprocessing for analysis
...
This section provides descriptive statistics of the app users.  
...
This section includes visualizations of key demographics. The age is separated into intervals in order to keep the users anonymous. 
```{r}
...
```
```{r}
# --- Visualize distribution of male and female users
...
subtask_dist <- data_bin %>%
  ...
  count(name = "n") %>%                           
  ungroup() %>%                                   
  complete(ID, subtask_completed = c(0, 1), fill = list(n = 0)) %>% 
...
### Visualizing number of completed tasks for each age group
```{r}
# --- Append age
...
# --- Box plot: Task engagement across age groups
...
# --- Mixed-effects logistic regression with random intercept for participants
...
In order to do so, we only include users who have engaged with the task
```{r}
